...
func exportQueriesFromStmtSummary(db *tidbHandler, specDB, dstFile string) error {
	query := `SELECT SCHEMA_NAME, QUERY_SAMPLE_TEXT FROM information_schema.cluster_statements_summary_history WHERE STMT_TYPE='Select' LIKE '%select%' and SCHEMA_NAME != 'NULL'`
	if specDB != "" {
		query = `SELECT SCHEMA_NAME, QUERY_SAMPLE_TEXT FROM information_schema.cluster_statements_summary_history WHERE SCHEMA_NAME != 'NULL' and STMT_TYPE='Select' AND SCHEMA_NAME='` + specDB + `'`
	}
